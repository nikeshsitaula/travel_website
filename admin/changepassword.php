          ...
          <div class="col-lg-12">
		   ...
		   <?php
                        if(isset($_POST['submit2']))
                        {
                         ...
                         try {
		$cpass=sha1($_POST['password1']);
		$npass=sha1($_POST['password']);
		$sql2="UPDATE `admindata` SET `password` = ? WHERE `customerdata`.`password` = ?"; 
		$query2 = $conn->prepare($sql2);
		$query2->bindParam(1,$npass,PDO::PARAM_STR);
		$query2->bindParam(2,$cpass,PDO::PARAM_STR);
		$query2->execute();
		if($query2->rowCount())
		{
			echo '<script> alert("Admin password has been Updated");</script>';
