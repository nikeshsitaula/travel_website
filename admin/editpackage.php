		...
		if(isset($_FILES['pimage']))
	{
			...
			if(!empty($name))
			{
				if(in_array($ext,$compare))
				{
					...
					if(move_uploaded_file($tmp_name,$location.$name))
					{
						...
						$sql="UPDATE `package` SET `package_name` = ?, `package_desc` = ?, `start_from` = ?, `img_path` = ?, `facility_highlight` = ? WHERE `package`.`id` = ?"; 
		$query = $conn->prepare($sql);
		$query->bindParam(1,$pname,PDO::PARAM_STR);
		$query->bindParam(2,$pdesc,PDO::PARAM_STR);
		$query->bindParam(3,$price,PDO::PARAM_STR);
		$query->bindParam(4,$loc_file,PDO::PARAM_STR);
		$query->bindParam(5,$facilities,PDO::PARAM_STR);
		$query->bindParam(6,$pid,PDO::PARAM_STR);
		$query->execute();
		if($query->rowCount())
		{
			echo '<script> alert("Package details has been Updated");</script>';
			header('Refresh:5;url=managepackage.php');
		...
		else
		{
			echo '<script> alert("Please Correct details, try again!");</script>';
				...
				{
		$sql2="UPDATE `package` SET `package_name` = ?, `package_desc` = ?, `start_from` = ?,`facility_highlight` = ? WHERE `package`.`id` = ?"; 
		$query2 = $conn->prepare($sql2);
		$query2->bindParam(1,$pname,PDO::PARAM_STR);
		$query2->bindParam(2,$pdesc,PDO::PARAM_STR);
	...
	if ($row = $query->fetch(PDO::FETCH_ASSOC))
{
				   ...
				   <div class="form-group ">
                      <label for="pid" class="control-label col-lg-2">Package ID <span class="required">*</span></label>
                      <div class="col-lg-10">
					...
					<div class="form-group ">
                      <label for="pname" class="control-label col-lg-2">Package Name <span class="required">*</span></label>
                      <div class="col-lg-10">
                        <input class=" form-control" id="pname" name="pname" value="<?php echo $row['package_name']; ?>" type="text" required/>
                      </div>
                    </div>
                    <div class="form-group ">
                      <label for="pdesc" class="control-label col-lg-2">Package Description <span class="required">*</span></label>
                      <div class="col-lg-10">
                        <input class=" form-control" id="pdesc" name="pdesc" value="<?php echo $row['package_desc']; ?>" type="text" required/>
                      </div>
                    </div>
					<div class="form-group ">
                      <label for="price" class="control-label col-lg-2">Start Price<span class="required">*</span></label>
                      <div class="col-lg-10">
                        <input class=" form-control" id="price" name="price" value="<?php echo $row['start_from']; ?>" type="text" required/>
                      </div>
                    </div>
					
					
					 <div class="form-group ">
                      <label for="pimage" class="control-label col-lg-2">Package Image<span class="required">*</span></label>
                      <div class="col-lg-10">
